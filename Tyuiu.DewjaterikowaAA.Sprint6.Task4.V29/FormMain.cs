...
namespace Tyuiu.DewjaterikowaAA.Sprint6.Task4.V29
{
    public partial class FormMain : Form
    {
        ...
        {
            MessageBox.Show("Таск 4 выполнила студентка ИИПБ-25-1 Девятерикова Анастасия Андреевна", "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void buttonDone_DAA_Click(object sender, EventArgs e)
        {
            try
            {
                int startStep = Convert.ToInt32(textBoxStartStep_DAA.Text);
                int stopStep = Convert.ToInt32(textBoxStopStep_DAA.Text);
                ...
                chartFunction_DAA.Series[0].Points.Clear();
                for (int i = 0; i <= len - 1; i++)
                {
                    this.chartFunction_DAA.Series[0].Points.AddXY(startStep, valueArray[i]);
                    textBoxResult_DAA.AppendText(valueArray[i] + Environment.NewLine);
                    startStep++;
                }
            }
            catch
        ...
        private void buttonSave_DAA_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                File.WriteAllText(path, textBoxResult_DAA.Text);
